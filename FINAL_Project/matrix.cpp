...
void        SolveTriangularSystemUP   (double *x, double *A, double *b, uint64_t n){
    ...
    for ( int column = n-1 ; column >= 0 ; column--){
        ...
        for ( int line = column-1 ; line >= 0 ; line--){
            ...
            b[line] = b[line] - coefficient*b[column];
        }
    }
    ...
    for (int column = 0; column < n - 1; column++) {
        ...
        for (int line = column; line < n - 1; line++) {
            ...
            for (int k = column; k < n; k++) {
                A[n * (line + 1) + k] -= A[n * column + k] * coefficient; //Multiply variable in matrix by coefficient.
            }
            A[n * (line + 1) + column] = coefficient;//Replace the lower part of the matrix, composed of 0, by the coefficient calculated in this same loop.
        }
    }
        return error; //If searchError returns true.
    }
...
double      Det         (double *A, uint64_t n) {
    double det = 1;
    for (int i = 0 ; i < n ; ++i) {
        det *= A[i * n + i]; //Select diagonals coefficients in matrix and multiply.
    }
    return det;
}
...
/*
 * Solves a system of linear equations Ax=b for a double-precision matrix A (size n x n).
 * Uses iterative descent algorithm.
 * After the procedure, x contains the solution of Ax=b.
 * We assume that x has been allocated outside the function.
 */
void        SolveTriangularSystemLOW   (double *x, double *A, double *b, uint64_t n){
    ...
    for ( int column = 0 ; column < n ; column++){
        ...
        for ( int line = column+1 ; line < n ; line++){
            coefficient = A[n*line + column]; //Find the coefficient to make all the coefficient in column j to 0.
            b[line] = b[line] - (coefficient*b[column]); //Subtract vector b at line with coefficient.
        }
    }
    for ( int index = 0 ; index < n ; index++){
        x[index] = b[index] ; //Solves equation at vector x such that Ax = b with pivot.
    }
}
...
/*
 * Solves a system of linear equations Ax=b, given a matrix A (size n x n) and vector b(size n).
 * This function uses LU system to solves matrix. SolveSystemLU calls decompLU, SolveTriangularSystemDOWN and SolveTriangularSystemUP and, returns a boolean variable:
...
bool        SolveSystemLU        (double *x, double *result, double *A, double *b, uint64_t n) {
    ...
    //Begin
    if (decompLU(A, n)) { //Test if decompLU return true.
        SolveTriangularSystemLOW(x, A, b, n); //Applies Down method
        SolveTriangularSystemUP(result, A, x, n); //Applies Up method
        return true;
    } else {
        return false; //If system can't be solved.
    }
